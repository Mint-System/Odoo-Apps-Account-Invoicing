...
class AccountMove(models.Model):
    ...
    def set_position(self):
        get_positions_from_orders = self.env['ir.config_parameter'].sudo().get_param('account.get_positions_from_orders')
        for move in self:
            position = 0
            for line in move.invoice_line_ids.filtered(lambda l: not l.display_type).sorted('sequence'):
